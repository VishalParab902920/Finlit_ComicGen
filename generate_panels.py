...
For each cartoon panel, you will write a description of it with:
 - the characters in the panel, they must be described precisely each time based on their provided description
 - the background of the panel
The description should be only word or group of word delimited by a comma, no sentence.
Always use the characters descriptions instead of their name in the cartoon panel description for every panel, avoid assuming contect for descriptions. Give everything in detail.
...
character_template = """
You are a comic creator.

You will be given short description of the characters of a scenario.
For each character in the scenario, write a detailed description with details of their attire, facial features, looks, hair style, body type, etc.
This description should be usable as a prompt for any image generation model to give consistent looking results.
You can not use the same description twice.
The text should not be more than 2 small sentences.
...
def generate_characters(characters):
    ...
    human_message_prompt = HumanMessagePromptTemplate.from_template(character_template)

    chat_prompt = ChatPromptTemplate.from_messages([human_message_prompt])

    chat_prompt.format_messages(characters=characters)

    result = model(chat_prompt.format_messages(characters=characters))

    print(result.content)

    return result.content

def generate_panels(scenario, characters):
    model = ChatOpenAI(model_name='gpt-3.5-turbo-0125')
    ...
    chat_prompt.format_messages(scenario=scenario, characters=characters)

    result = model(chat_prompt.format_messages(scenario=scenario, characters=characters))
